		...
		foreach ($emap_items as $row => $values) {
			if($values->username != "" || $values->facebook_name != "" || $values->twitter_name != "" ) {
				...
				$values_array[$row] = "{
					...
					options:{
						icon: '" . get_template_directory_uri() . "/images/black-marker-middle.png'
					}
	...
	?>
	 ...
	 function et_add_marker_update() {
			...
			var updateInfo = new Array;
		 	...
		 	for ( var i = 0 ; i < progInfo.length ; i++) {
			 	var latitude = progInfo[i].lat;
			 	var longitude = progInfo[i].lgt;
			 	updateInfo[i] = "{ id : 'et_marker_'  +  i,latLng:[" + latitude  + " , " + longitude  + "]}";
			 		
		 	}		
		 
			//var oIcon = "<?php echo get_template_directory_uri(); ?>/images/red-marker.png";
			var oAnimation = google.maps.Animation.DROP;
			var mIndex = progInfo.length - 1;
			var latitude = progInfo[progInfo.length - 1].lat;		
			var longitude = progInfo[progInfo.length - 1].lgt;		

			var marker_content;
			marker_content = "<div id='et_marker_'" + mIndex + " class='et_marker_info'>";
			marker_content += "<div class='location-title'> <h2>" + progInfo[mIndex].title + "</h2> <div class='listing-info' ><p>" + progInfo[mIndex].username + "</p></div> </div>"; 
			marker_content += "<div class='location-rating'><span class='et-rating'><span ></span></span></div> </div>";
			
			$et_main_map.gmap3({
				marker : {
					values:[updateInfo.join(',')],
					/* events:{
						click:function(marker, event, context){
							marker.setAnimation(0);
							//marker.setAnimation( google.maps.Animation.BOUNCE);
						...
						mouseover: function( marker, event, context ){
							var overlay = $(this).gmap3({get:{id: "overlay_" + mIndex}});
							if(overlay){
								overlay.show();
								$( "#et_marker_" + mIndex ).css( { 'display' : 'block', 'opacity' : 0 } ).stop(true,true).animate( { bottom : '15px', opacity : 1 }, 0 );
							}
							else {
								$(this).gmap3(
          						{			
								...
								var overlay = $(this).gmap3({get:{id: "overlay" + mIndex}});
								setTimeout(function(){
									overlay.show();	
								$( "#et_marker_" + mIndex ).css( { 'display' : 'block', 'opacity' : 0 } ).stop(true,true).animate( { bottom : '15px', opacity : 1 }, 0 );	
								},100);
						...
						mouseout: function( marker, event, context ){
							...
							if(overlay){
								overlay.hide();
								$( "#et_marker_" + mIndex ).stop(true,true).animate( { bottom : '50px', opacity : 0 }, 0, function() {
									$(this).css( { 'display' : 'none' } );
								});
							} else{
								$(this).gmap3(
				...
				options:{
					icon: "<?php get_template_directory_uri()?>/images/black-marker-middle.png"
				}
			...
			if(xmlhttp.readyState == 4 && xmlhttp.status == 200)
			{
				...
				if( progInfo.length == emapinfo.length - 1 )
				{
					...
					for ( var index = progInfo.length; index < emapinfo.length-1 ; index++ ) {
						...
						var oIcon = "<?php echo get_template_directory_uri(); ?>/images/black-marker-middle.png";
						var oAnimation = google.maps.Animation.DROP;
						var oCenter = [progInfo[index].lat , progInfo[index].lgt];
						var marker_id = 'et_marker_' + index;
						var overlay = 'overlay_' + index;

						//et_add_marker_update();
						
					    temp[index] = {pid:progInfo[index].pid , cid:progInfo[index].cid , title:progInfo[index].title , username:progInfo[index].username , rating:progInfo[index].vote};
					    $et_main_map.gmap3({	
							marker : {
								id : marker_id,
											...
											$(this).gmap3(
			          						{			
												overlay:{
													...
													latLng : [marker.position.d, marker.position.e],
													options:{
														content : '<div id="' + context.id + '" class="et_marker_info"><div class="location-description"> <div class="location-title"> <h2>' + temp[text_id].title + '</h2> <div class="listing-info" ><p>' + temp[text_id].username +'</p></div> </div> <div class="location-rating"><span class="et-rating"><span style="width:' + (temp[text_id].rating * 17) + 'px;"></span></span></div> </div> <!-- .location-description --> </div> <!-- .et_marker_info -->',
														offset:{
															y:-42,
															x:-120
														}
											...
											$(this).gmap3(
			          						{			
												overlay:{
													...
													latLng : [marker.position.d, marker.position.e],
													options:{
														content : '<div id="' + context.id + '" class="et_marker_info"><div class="location-description"> <div class="location-title"> <h2>' + temp[text_id].title + '</h2> <div class="listing-info" ><p>' + temp[text_id].username +'</p></div> </div> <div class="location-rating"><span class="et-rating"><span style="width:' + (temp[index].rating * 17) + 'px;"></span></span></div> </div> <!-- .location-description --> </div> <!-- .et_marker_info -->',
														offset:{
															y:-42,
															x:-60
														}
