(function ($) {
	$.widget("ui.rcarousel", {
		_create: function() {
			...
			$root
				.addClass(혻"ui-carousel"혻)
				.children()
				.wrapAll(혻"<div class='wrapper'></div>" );
			
			// save all children of root element in �쁯aths��array
			this._saveElements();
		...
		_addElement: function( jQueryElement, direction )혻{
			var $root = $( this.element ),
				$content = $root.find(혻"div.wrapper"혻),
				options = this.options;
		...
		_autoMode: function( direction ) {
			var options = this.options,
				data = $( this.element ).data(혻"data"혻);
		...
		_generatePages: function() {
			...
			// creates 3 pages for �쁵isible: 5��and �쁲tep: 4��
			// [ABCDE], [EFGHI], [FGHIJ]
			...
			function _paginate() {
				...
				while ( !_islastPage(data.pages[data.pages.length - 1]) || _isBeginning ) {
					...
					// when we run ouf of elements (_end - _start <혻_visible) we must add the difference at the begining
					// in our example the 3rd page is [FGHIJ] and J element is added in the second step
					// first we add [FGHI] as old elements
					// we must assure that we have always �쁵isible��(5 in our example) elements
		...
		_goToPrevPage: function( by ) {
			...
			// CD page was shown (from 5 elements) ��theSame��is true because
			// we compare the same
			...
			$( _page ).each(
				function( i, el )혻{
					if ( el.get(0) === $(_oldPage[i]).get(0) )혻{
						_theSame = true;
					} else {
						_theSame = false;
					}
			...
			if ( data.appended && _theSame )혻{
				if ( data.pageIndex === 0 )혻{
			...
			// for [ABCDFGHIJ] elements there are 3 pages for �쁵isible��= 6 and
			// �쁲tep��= 2: [ABCDEF], [CDEFGH] and [EFGHIJ]; going from the 3rd
		...
		_goToNextPage: function( by ) {
			...
			// ABCD page was shown (from 5 elements) ��theSame��is true because
			// we compare the same
			...
			// for [ABCDFGHIJ] elements there are 3 pages for �쁵isible��= 6 and
			// �쁲tep��= 2: [ABCDEF], [CDEFGH] and [EFGHIJ]; going from the 2nd
			...
			if ( options.orientation === "horizontal" )혻{
		...
		_saveElements: function()혻{
			var $el,
				$root = $( this.element ),
				$elements = $root.find( "div.wrapper"혻).children(),
				data = $root.data( "data" );
			...
			$elements.each(
				function( i, el ) {
					...
					// keep element�셲 data and events
