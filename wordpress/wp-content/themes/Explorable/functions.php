<?php
update_option('siteurl', DOMAIN_ROOT);
...
function get_vod_title($vod_path) {
	$strVodPath = explode(',', trim($vod_path));
	return $strVodPath[0];
}
function et_get_emap_items() {
	global $wpdb;
	...
	return $wpdb->get_results("SELECT   	
											...
											lu.picture_path as userpic,
											lu.facebook_id as facebook_id,
											lu.twitter_id as twitter_id,
										...
										on
											lu.wp_user_id = wu.ID
									WHERE
											(lp.start_time >= DATE_SUB('" . $currentTime . "', INTERVAL 1 DAY)
									AND (lp.end_time is null OR lp.end_time <= '".$currentTime."' AND lp.vod_enable='1')");
}


function videoList($channelid) {
	global $wpdb;
	$fetch_row_array =  $wpdb->get_results(" select * FROM wp_program WHERE channel_id = '.$channelid.'");
	foreach($fetch_row_array as $key => $row) {
		$column = $row;
	}
/* 	return $wpdb->get_results(" select * FROM wp_program WHERE channel_id = '.$channelid.'"); */
	return $column;
}


function et_get_usernamebyid($userid) {
	global $wpdb;
	return $wpdb->get_results("SELECT wu.user_nicename,lu.facebook_name,lu.twitter_name FROM wp_user_yep lu left join wp_users wu on lu.wp_user_id = wu.ID WHERE lu.user_id = '".$userid."'");
}

function et_get_emap_location($lat, $lgt) {
	$geocode=file_get_contents('http://maps.googleapis.com/maps/api/geocode/json?latlng='.$lat.','.$lgt.'&sensor=false');

    $output= json_decode($geocode);

    $retLocation = "";
    for($j=0;$j<count($output->results[0]->address_components);$j++){
    	$retLocation .= $output->results[0]->address_components[$j]->long_name.' ';
    }
...
function et_attach_bg_images() {
	...
	$header_bg = et_get_option( 'explorable_bg_image', $template_directory .  '/images/global_map.jpg' );
...
if ( function_exists( 'get_custom_header' ) ) {
	...
	function et_explorable_add_customizer_css(){ ?>
		<style>
			...
			#top-navigation a { color: #000000; }
			#top-navigation li.current-menu-item > a, .et_mobile_menu li.current-menu-item > a { color: #000000; }
			
			.nav li ul { background-color: #f0f0f0; }
