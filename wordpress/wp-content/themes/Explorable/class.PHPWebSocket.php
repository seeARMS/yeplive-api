...
class PHPWebSocket
{
	...
	/*
		$this->wsClients[ integer ClientID ] = array(
			...
			12 => boolean  isRegistered						  // custom field by me chat client is registered?
			13 => string   UserId							  // custom field by me chat client user id
			14 => string   ChannelId						  // custom field by me chat client channel id
		)
	...
	*/
	...
	function wsProcessClientMessage($clientID, $opcode, &$data, $dataLength) {
		...
		if ($opcode == self::WS_OPCODE_PING) {
            ...
            $this->log("Received Ping command");
